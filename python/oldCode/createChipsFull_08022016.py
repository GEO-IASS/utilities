...
def cutChipFromMosaic(rasterFile, shapeFileSrc, outlineSrc,outputDirectory='', outputPrefix='clip_',
                      clipSizeMX=100, clipSizeMY=100, numBands=8, clipRaster=True, clipGeoJson=True):
...
if __name__ == '__main__':
    ...
    outlineSrc = 'Rio_West_outline.geojson'

    buildingsSrc = 'Rio_West_Buildings_Clipped.geojson'
    outdirBase = '/usr/local/share/data/spacenetIntermediate/spacenet_08242016'
    ...
    for image in rastList:
        rasterFile = os.path.join('/usr/local/share/data/spacenetIntermediate/mosaic', image)
        print rasterFile

        print " ========================================= Let's make some chips! ========================================= "

        outdir = os.path.join(outdirBase, 'chips3band')
        cutChipFromMosaic(rasterFile, buildingsSrc, outlineSrc, outputDirectory=outdir, outputPrefix='clip2_',
                              clipSizeMX=200, clipSizeMY=200, numBands=3, clipRaster=False)



        rasterFile = os.path.join('/usr/local/share/data/spacenetIntermediate/mosaic_8band', image)
        outdir = os.path.join(outdirBase, 'chips8band')
        cutChipFromMosaic(rasterFile, buildingsSrc, outlineSrc, outputDirectory=outdir, outputPrefix='clip2_',
                          clipSizeMX=200, clipSizeMY=200, numBands=8, clipRaster=False)
